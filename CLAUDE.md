...
- 36 articles (7 original + 27 WP migration + 2 Bitan Finance)
- 23 FAQs (5 original + 18 Bitan Finance)
- 11 services — unique body content, companies-first, no duplicate patterns
- 7 parent categories + 10 subcategories — two-row filter UI
...
- Client logo conveyor belt — built with CSS scroll animation, placeholders showing, real logos pending
- AI-generated article images for all articles (OpenAI API, script at scripts/generate-article-images.ts)
...
- About page: Avi + Ron Bitan (brothers), second generation to father Shlomo Bitan
- Trust bar: 30+ שנות ניסיון · דור שני · רו"ח + משפטנים · תל אביב
- Homepage scroll journey: Hero → TrustBar → Services → About → Process → Testimonials (Elfsight) → Client Logos → Knowledge Preview → TrustModule → FAQ → CTA → Footer

## Schemas (13)
article · author · category (with parent self-reference for subcategories) · tag · service · faq · testimonial · contactLead · homePage · aboutPage · legalPage · siteSettings · clientLogo
...
- scripts/generate-article-images.ts — Reusable OpenAI image generation
...
- Client logos are placeholders (pending from founders)
- Google Maps on /contact may show rejection if API key referrer not configured for domain
- No tests, no CI/CD
- Article image gen quality is functional not premium (V4 upgrade planned)
